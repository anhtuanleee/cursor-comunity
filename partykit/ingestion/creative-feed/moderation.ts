...
export type ModerationResult =
  | {
      decision: "approved";
      reason: "automated-creative-quality-pass" | "motion-priority-pass";
    }
...
const GENERIC_AUTHORS = /^(?:admin|author|editor|staff|team|unknown)$/i;
...
function hasCredibleAuthor(value: string | null): boolean {
  if (!value) return false;
  const normalized = normalize(value);
  const compact = normalized.replace(/\s/g, "");
  return compact.length >= 2 && compact.length <= 120 && !GENERIC_AUTHORS.test(normalized);
}
...
export function moderateCreativeEntry(
  ...
  if (!hasCredibleAuthor(entry.author)) return { decision: "rejected", reason: "missing-or-invalid-author" };
  ...
  return {
    decision: "approved",
    reason: motion ? "motion-priority-pass" : "automated-creative-quality-pass",
  };
}
